...
void MX_TIM2_Init(void)
{
  ...
  HAL_TIM_IC_Start(&htim2, TIM_CHANNEL_1);             	/* 开始捕获TIMx的通道y */
...
uint32_t gtim_timx_cnt_chy_get_count(void)
{
    ...
    count += __HAL_TIM_GET_COUNTER(&htim2);  /* 加上当前CNT的值 */
    return count;
}
...
{
    __HAL_TIM_DISABLE(&htim2);        	/* 关闭定时器TIMX */
    g_timxchy_cnt_ofcnt = 0;           	/* 累加器清零 */
    __HAL_TIM_SET_COUNTER(&htim2, 0); 	/* 计数器清零 */
    __HAL_TIM_ENABLE(&htim2);         	/* 使能定时器TIMX */
}
