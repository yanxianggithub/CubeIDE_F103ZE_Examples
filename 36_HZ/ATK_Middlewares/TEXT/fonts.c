...
static uint8_t fonts_update_fontx(uint16_t x, uint16_t y, uint8_t size, uint8_t *fpath, uint8_t fx, uint16_t color)
{
    ...
    tempbuf = (uint8_t *)mymalloc(SRAMIN, 4096);               /* 分配4096个字节空间 */

    if (tempbuf == NULL)rval = 1;
    tempbuf2 = tempbuf;		/* 使用Debug模式编译后的结果tempbuf的值会莫名改变，所以这里另外定义一个tempbuf2防止被优化 */
    ...
    if (rval == 0)
    {
        ...
        while (res == FR_OK)   /* 死循环执行 */
        {
            ...
            offx += bread;
            fonts_progress_show(x, y, size, fftemp->fsize, offx, color);    /* 进度显示 */
